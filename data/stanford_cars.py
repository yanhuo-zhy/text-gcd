...
from config import car_root, car_tag_root
...
class CarsDataset_LENS(CARS_Base):
    def __init__(self, train=True, limit=0, data_dir=car_root, transform=None):
        super().__init__(train, limit, data_dir, transform)
...
class CarsDataset(CARS_Base):
    def __init__(self, train=True, limit=0, data_dir=car_root, tag_root=None, transform=None, text_transform=None):
    ...
    def __getitem__(self, idx):
        ...
        return image, target, image_id, tag_token, idx
...
def get_scars_datasets(train_transform, 
    ...
    whole_training_set = CarsDataset(data_dir=car_root, tag_root=tag_root, transform=train_transform, text_transform=text_transform, train=True)
    ...
    test_dataset = CarsDataset(data_dir=car_root, tag_root=tag_root, transform=test_transform, text_transform=None, train=False)
