...
from config import herbarium_dataroot, herbarium_tag_root

from clip import clip

class HerbariumDataset19_Base(torchvision.datasets.ImageFolder):

    def __init__(self, root=os.path.join(herbarium_dataroot, 'small-train'), transform=None):

        # Process metadata json for training images into a DataFrame
        super().__init__(transform=transform, root=root)

        self.uq_idxs = np.array(range(len(self)))

class HerbariumDataset19_LENS(HerbariumDataset19_Base):
    def __init__(self, root=os.path.join(herbarium_dataroot, 'small-train'),transform=None):
...
class HerbariumDataset19(HerbariumDataset19_Base):
    def __init__(self, tag_root, root=os.path.join(herbarium_dataroot, 'small-train'), transform=None, text_transform=None):
...
def get_herbarium_datasets(train_transform, 
    ...
    train_dataset = HerbariumDataset19(tag_root=tag_root, transform=train_transform, text_transform=text_transform,
    ...
    # Get test dataset
    test_dataset = HerbariumDataset19(tag_root=tag_root, transform=test_transform, text_transform=None,
