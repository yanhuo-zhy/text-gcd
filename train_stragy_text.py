...
from model import CustomCLIP_TwoText, CustomCosineAnnealingLR, ImageViewGenerator, TextViewGenerator_TwoText, load_clip_to_cpu
from data.augmentations import get_transform
from utils import init_experiment, get_pseudolabel_twotext, evaluate_two_text, evaluate_weighted_twotext
...
def train_one_epoch(args, logger, writer, loader, model, optimizer, scheduler, epoch, image_to_class_map, image_to_class_map_i):
    ...
    for batch_idx, (_, labels, img_id, tag_token, _, mask) in enumerate(tqdm(loader, desc="Training")):
        mask = mask[:, 0]
        ...
        # print(images.shape)
        text1 = torch.cat(tag_token.chunk(4)[:2], dim=0)
        text2 = torch.cat(tag_token.chunk(4)[2:], dim=0)

        logits_image1, logits_image2, image_features1, image_features2 = model(text1, text2)
        ...
        sup_logits = torch.cat([f[mask] for f in (logits_image1 / 0.1).chunk(2)], dim=0)
        sup_labels = torch.cat([labels[mask] for _ in range(2)], dim=0)
        loss_cls = nn.CrossEntropyLoss()(sup_logits, sup_labels)

        sup_logits_text = torch.cat([f[mask] for f in (logits_image2 / 0.1).chunk(2)], dim=0)
        loss_cls += nn.CrossEntropyLoss()(sup_logits_text, sup_labels)        

        #-----------------------------------------------------SimGCD cluster loss------------------------------------------------#

        student_out = logits_image1 / 0.05
        teacher_out = logits_image1.detach()
        ...
        avg_probs = (logits_image1 / 0.05).softmax(dim=1).mean(dim=0)
        me_max_loss = - torch.sum(torch.log(avg_probs**(-avg_probs))) + math.log(float(len(avg_probs)))
        loss_cluster += 2 * me_max_loss
        #--------------------------------------------------------------------
        student_out_text = logits_image2 / 0.05
        teacher_out_text = logits_image2.detach()
        ...
        avg_probs_text = (logits_image2 / 0.05).softmax(dim=1).mean(dim=0)
        ...
        if image_to_class_map:
            for idx, id in enumerate(img_id+img_id):
                if id in image_to_class_map:
                    ...
                    selected_logits.append(logits_image1[idx])

        # 如果存在伪标签，计算交叉熵损失
        if pseudo_labels:
            pseudo_labels_tensor = torch.tensor(pseudo_labels).to(logits_image1.device)  # 将列表转化为张量，并移到适当的设备上
            ...
            loss_pseduo = torch.tensor(0.0,device=logits_image1.device)
        # #-------------------------------------------
        if image_to_class_map_i:
            ...
            for idx, id in enumerate(img_id+img_id):
                if id in image_to_class_map_i:
                    ...
                    selected_logits_i.append(logits_image2[idx])

            # 如果存在伪标签，计算交叉熵损失
            if pseudo_labels_i:
                pseudo_labels_i_tensor = torch.tensor(pseudo_labels_i).to(logits_image2.device)  # 将列表转化为张量，并移到适当的设备上
                ...
                loss_pseduo_i = torch.tensor(0.0,device=logits_image2.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_name', type=str, default='Two_Text_test')
    parser.add_argument('--seed_num', type=int, default=1)
    parser.add_argument('--evaluate', type=bool, default=False)
    parser.add_argument('--dataset_name', type=str, default='cub', help='options: cifar10, cifar100, imagenet_100, cub, scars, aircraft, herbarium_19, pets, flowers, food')
    ...
    clip_model1 = load_clip_to_cpu(args.backbone_name).float()
    clip_model2 = load_clip_to_cpu(args.backbone_name).float()
    ...
    model = CustomCLIP_TwoText(clip_model1, clip_model2, args.num_classes).to(args.device)
    ...
    for name, param in model.named_parameters():
        if "model1.transformer.resblocks.11" in name:
            param.requires_grad_(True)
        if "model2.transformer.resblocks.11" in name:
            param.requires_grad_(True)
    ...
    # train_transform = ImageViewGenerator(base_transform=train_transform, n_views=4)
    text_transform = TextViewGenerator_TwoText(alpha_sr=args.alpha_sr, alpha_ri=args.alpha_ri, alpha_rs=args.alpha_rs, alpha_rd=args.alpha_rd, seed=args.seed_num)
    # text_transform = None
    ...
    for epoch in range(start_epoch, args.epochs):
        image_to_class_map, image_to_class_map_i = get_pseudolabel_twotext(model, test_loader, pseudo_num=pseudo_num)
        ...
        if epoch == 0:
            total_acc_text, old_acc_text, new_acc_text, total_acc_image, old_acc_image, new_acc_image = evaluate_two_text(model, test_loader, train_classes=args.train_classes)
        ...
        total_acc_w, old_acc_w, new_acc_w = evaluate_weighted_twotext(model, test_loader, train_classes=args.train_classes)
