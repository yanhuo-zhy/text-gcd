...
Date: 2023-11-11 20:55:06
LastEditors: yanhuo 1760331284@qq.com
LastEditTime: 2023-11-11 20:58:34
FilePath: \text-gcd\train_stragy.py
...
from model import CustomCLIP_TwoImage, CustomCosineAnnealingLR, ImageViewGenerator, TextViewGenerator, load_clip_to_cpu
from data.augmentations import get_transform
from utils import init_experiment, get_pseudolabel_twoimage, evaluate_two_images, evaluate_weighted_twoimage
...
def train_one_epoch(args, logger, writer, loader, model, optimizer, scheduler, epoch, image_to_class_map, image_to_class_map_i):
    ...
    for batch_idx, (images, labels, img_id, _, _, mask) in enumerate(tqdm(loader, desc="Training")):
        ...
        # print(images.shape)
        images1 = torch.cat(images.chunk(4)[:2], dim=0)
        images2 = torch.cat(images.chunk(4)[2:], dim=0)

        logits_image1, logits_image2, image_features1, image_features2 = model(images1, images2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_name', type=str, default='Two_Image_test')
    ...
    model = CustomCLIP_TwoImage(clip_model1, clip_model2, args.num_classes).to(args.device)
    ...
    for epoch in range(start_epoch, args.epochs):
        image_to_class_map, image_to_class_map_i = get_pseudolabel_twoimage(model, test_loader, pseudo_num=pseudo_num)
        ...
        if epoch == 0:
            total_acc_text, old_acc_text, new_acc_text, total_acc_image, old_acc_image, new_acc_image = evaluate_two_images(model, test_loader, train_classes=args.train_classes)
        ...
        total_acc_w, old_acc_w, new_acc_w = evaluate_weighted_twoimage(model, test_loader, train_classes=args.train_classes)
